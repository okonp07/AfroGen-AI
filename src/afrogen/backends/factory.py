...
def create_backend(name: str, image_size: int, latent_shape: tuple[int, int]):
    ...
    if normalized in {"trained", "diffusion", "hybrid"}:
        return TrainedAfroGenBackend(name=normalized, image_size=image_size, latent_shape=latent_shape)
